...
class JoinedController extends Controller
{
    ...
    {
        return AppointmentRequests::with('appointments')
            ->where('customer_id', auth()->id())
            ->orderBy('created_at', 'desc')
            ->get();
    }

    public function appointmentRequestList()
    {
        return AppointmentRequests::with(['appointments', 'customer'])
        ...
        if ($specialtyId) {
            try {
                ...
                if (!$existing) {
                    ...
                    $results['messages'][] = 'Specialty already exists for this staff.';
                }
            } catch (\Exception $e) {
                $results['messages'][] = 'Error adding specialty: ' . $e->getMessage();
            }
            ...
            try {
                ...
                if ($user) {
                    ...
                    $results['messages'][] = $activeStatus
                        ? 'Staff marked as active.'
                        : 'Staff marked as inactive.';
                } else {
                    $results['messages'][] = 'Staff member not found.';
                }
            } catch (\Exception $e) {
                $results['messages'][] = 'Error updating active status: ' . $e->getMessage();
            }
        }
