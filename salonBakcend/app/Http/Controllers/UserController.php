...
class UserController extends Controller
{
    ...
    public function register(Request $request)
    {
        ...
        // ✅ Send the account details email only for staff accounts
        if ($user->role === 'staff') {
            try {
                Mail::to($user->email)->send(new StaffAccountCreated([
                    ...
                    'email'        => $user->email,
                    'password'     => $request->password, // plaintext at creation time only
                    'role'         => $user->role,
                    'phone_number' => $user->phone_number,
                ]));
            } catch (\Exception $e) {
                // Don't fail the whole request — the account is already created.
                Log::error('Staff welcome email failed: ' . $e->getMessage(), [
                    'user_id' => $user->id,
                    'email'   => $user->email,
                ]);
            }
